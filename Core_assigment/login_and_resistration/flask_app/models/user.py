...
# ********************************************
    ...
    def insert_user(cls,data):
        ...
        return connectToMySQL("login_and_resigtration").query_db(query, data)
...
# ********************************************
    ...
    def get_by_email(cls,data):
        ...
        user_db = connectToMySQL("login_and_resigtration").query_db(query, data)
    ...
    def validate_user(user):
        ...
        is_valid = True
        # this basicaly set the form to be true AKA "ok to pass", then we set a buch of condition and i one of those conditions failed the we set it to be is_valide = false therefoer no login(plus we flash our message)
        ...
        if  not email_reg.match(user["email"]):
            ...
            query = "SELECT * FROM users WHERE email = %(email)s"
            email_check = connectToMySQL("login_and_resigtration").query_db(query, data)
